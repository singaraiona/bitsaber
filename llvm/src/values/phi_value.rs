...
impl<'a> PhiValue<'a> {
    ...
    pub fn count_incoming(self) -> u32 { unsafe { LLVMCountIncoming(self.as_llvm_value_ref()) } }
    ...
    pub fn get_incoming(self, index: u32) -> Option<(Value<'a>, BasicBlock<'a>)> {
        ...
        let basic_block = unsafe {
            BasicBlock::new(LLVMGetIncomingBlock(self.as_llvm_value_ref(), index)).expect("Invalid BasicBlock")
        };
